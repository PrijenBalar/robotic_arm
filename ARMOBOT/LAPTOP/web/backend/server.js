...
// --- Pico W TCP Bridge ---
// The arm answers at ONE of these depending on network mode:
//   192.168.137.50 — arm joined the PC's Mobile Hotspot (station mode)
//   192.168.4.1    — PC joined the arm's own AP 'RoboticArm_AP' (direct mode)
// The bridge tries each candidate in turn with a short connect timeout until
// one answers; HTTP forwarding (/pico/*) follows the active address.
const PICO_CANDIDATES = process.env.PICO_IP
  ? [process.env.PICO_IP]
  : ['192.168.137.50', '192.168.4.1'];
const PICO_PORT = parseInt(process.env.PICO_PORT) || 81;
const PICO_CONNECT_TIMEOUT_MS = 3000;
let picoSocket = null;
let picoIpIdx = 0;
let activePicoIp = PICO_CANDIDATES[0];
...
// --- Local jog relay -------------------------------------------------------
// The Pico's port-81 jog server accepts only ONE TCP client, and the backend
// holds it. To let other tools (e.g. test/base_scan_stop.py) share the arm
// WITHOUT fighting over that single socket, the backend re-exports the same
// line protocol on a local port: commands written by a relay client are
// forwarded to the Pico, and the Pico's raw stream (telemetry JSON + SPEED_OK)
// is fanned out to every relay client. Point base_scan_stop.py at this port
// instead of 192.168.137.50:81 (see its --via-backend flag).
// NOTE: this shares the connection, not arbitration — only ONE source should
// command motion at a time, or jog commands from the web UI and the script
// will interleave on the firmware (last write wins).
...
function connectToPico() {
  ...
  const ip = PICO_CANDIDATES[picoIpIdx % PICO_CANDIDATES.length];
  picoSocket = new net.Socket();
  picoSocket.setTimeout(PICO_CONNECT_TIMEOUT_MS);

  picoSocket.connect(PICO_PORT, ip, () => {
    picoSocket.setTimeout(0);   // connected — connect timeout no longer applies
    activePicoIp = ip;
    console.log(`Connected to Pico W TCP Bridge at ${ip}:${PICO_PORT}`);
  });

  picoSocket.on('timeout', () => {
    // connect stalled — drop fast and let the retry loop try the next candidate
    picoSocket.destroy(new Error(`connect timeout ${ip}:${PICO_PORT}`));
  });

  picoSocket.on('data', (data) => {
    // Fan the raw Pico stream out to relay clients verbatim (telemetry JSON
    // and SPEED_OK acks) so they see exactly what a direct connection would.
    for (const c of relayClients) {
  ...
  picoSocket.on('close', () => {
    ...
    picoSocket = null;
    picoIpIdx++;   // alternate hotspot/direct-AP address on every retry
...
// --- Pico HTTP forward (/pico/* → active Pico address, port 80) -------------
// The frontend never talks to a fixed arm IP: Vite proxies /pico here, and
// the backend forwards to whichever address the TCP bridge last reached
// (hotspot 192.168.137.50 vs direct-AP 192.168.4.1).
app.use('/pico', (req, res) => {
  const path = req.originalUrl.replace(/^\/pico/, '') || '/';
  const preq = http.request({ host: activePicoIp, port: 80, path, method: 'GET' }, (presp) => {
    res.status(presp.statusCode || 200);
    presp.pipe(res);
  });
  // /calibrate blocks the Pico's reply until homing completes — allow 120 s.
  preq.setTimeout(120000, () => preq.destroy(new Error('Pico HTTP timeout')));
  preq.on('error', (e) => {
    if (!res.headersSent) {
      res.status(502).json({ error: `Pico unreachable at ${activePicoIp}: ${e.message}` });
    }
  });
  preq.end();
});
...
// --- Jog relay server: lets external tools share the Pico's single-client
//     port 81 through the backend (see relayClients comment above). ----------
const RELAY_PORT = parseInt(process.env.RELAY_PORT) || 8181;
const relayServer = net.createServer((client) => {
  relayClients.add(client);
  const who = `${client.remoteAddress}:${client.remotePort}`;
  console.log(`[Relay] Client connected ${who} (${relayClients.size} total)`);

  client.on('data', (buf) => {
    // Forward command lines (START_/STOP/SPEED_...) straight to the Pico.
    if (picoSocket && !picoSocket.destroyed) picoSocket.write(buf);
  });

  const drop = () => {
    if (relayClients.delete(client)) {
      console.log(`[Relay] Client gone ${who} (${relayClients.size} total)`);
    }
  };
...
relayServer.listen(RELAY_PORT, () => {
  console.log(`Jog relay listening on tcp://0.0.0.0:${RELAY_PORT} -> Pico [${PICO_CANDIDATES.join(' | ')}]:${PICO_PORT}`);
});
