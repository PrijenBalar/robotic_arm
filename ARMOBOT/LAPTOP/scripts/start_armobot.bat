@echo off
rem This script lives in scripts/ -- cd up to the repo root first.
cd /d "%~dp0.."
echo =========================================
...
start "ARMOBOT Backend" cmd /k "cd web\backend && npm start"

echo [2/3] Starting Frontend Server (Port 5173)...
start "ARMOBOT Frontend" cmd /k "cd web\frontend && npm run dev"
