...
# ── Networking ───────────────────────────────────────────────────────────────
# WiFi (station + AP fallback) lives in netboot.py — the single source of truth.
# boot.py brings it up robustly at cold boot; main.py reuses that interface (and
# brings it up itself when run directly from Thonny).  Edit SSIDs / static IP /
# AP credentials in netboot.py, NOT here.
import netboot
...
# Station WiFi + AP fallback now live in netboot.py (see the import above).
...
def jog_motion(j, d, tcp_sock):
    ...
    Starts j.stepper.free_run() then loops until any stopping condition:
      • limit switch pressed (debounced polling via limit_pressed())
    ...
    while True:
        ...
        # Stopping condition 1: limit switch (debounced poll)
        # Stopping condition 2: emergency button
        # Stopping condition 3: soft degree limits
        if limit_pressed() or emergency_btn.value() == 1 or (d == 1 and j.currentDegree >= j.maxDegree) or (d == 0 and j.currentDegree <= j.minDegree):
            j.stepper.stop()
            break
...
def switch_pressed(sw):
    """
    Debounced read of one limit switch: True only if the pin stays LOW for
    5 consecutive reads spanning ~1 ms.

    Edge IRQs are deliberately NOT used: EMI from the stepper drivers induces
    microsecond spikes on the L1/L2 switch lines (measured ~444 and ~250
    phantom falling edges per second at idle), which fired the old ISRs
    thousands of times per second — starving the CPU and blocking USB/Thonny.
    A real press holds the pin LOW for milliseconds and always passes this
    filter; a noise spike never does.
    """
    if sw.value() == 1:
        return False
    for _ in range(5):
...
def limit_pressed():
    """Debounced check of all three limit switches; latches limit_triggered."""
    global limit_triggered
    for sw in (limit_switch_1, limit_switch_2, limit_switch_3):
...
def  move_stepper(target_deg, joint: Joint):
    ...
    while joint.stepper._pos != joint.stepper._target:
        if limit_pressed() or check_emergency():
            joint.stepper.stop()
...
# ─────────────────────── REAL-TIME (NON-BLOCKING) TARGET ─────────────────────
def set_joint_target(joint: Joint, axis, angle):
    """
    REAL-TIME, NON-BLOCKING absolute-angle set.

    Unlike move_stepper(), this does NOT poll to completion: it applies the
    J2/J3 dynamic coupling, clamps to the joint's limits, points the
    timer-driven stepper at the new target, and returns IMMEDIATELY.  The main
    loop keeps accepting commands and streaming telemetry, so a camera loop can
    refresh the target every frame.  Returns False if blocked by a limit.
    """
    if limit_triggered:
        return False
    # Dynamic kinematic coupling — same as the HTTP /stepper handler.
    if axis == 2:
        solve_d3(angle)
    elif axis == 3:
        solve_d2(angle)
    angle = max(min(angle, joint.maxDegree), joint.minDegree)
    joint.stepper.speed(joint._jog_sps)
    joint.stepper.target_deg(angle)
    return True


# ─────────────────────────── CALIBRATION ─────────────────────────────────────
def _calib_abort(j1, j2, j3, reason=""):
    """Stop all motors after calibration abort."""
...
def calibrate_steppers(j1: Joint, j2: Joint, j3: Joint):
    ...
    Debounced direct pin polling is used so that an already-pressed limit
    switch is detected immediately on the first poll and EMI spikes on the
    switch lines are ignored.
    """
    ...
    while any(s != DONE for s in state):
        ...
        for i in range(3):
            if state[i] == SEEKING:
                if switch_pressed(switches[i]):       # debounced: held LOW → hit
...
if __name__ == "__main__":
    # Network is normally established by boot.py via netboot (station WiFi with
    # AP fallback).  When main.py is run directly (e.g. from Thonny, where
    # boot.py does NOT run), bring it up here.  Either way `net` is the active
    # STA-or-AP interface.
    net = netboot.iface or netboot.bring_up_network()[0]
    ...
    # Limit switches are polled with debounce (switch_pressed / limit_pressed)
    # instead of edge IRQs — see switch_pressed() for why.
    ...
    # ── Gateway keepalive (STA mode) ─────────────────────────────────────────
    # Windows Mobile Hotspot expires the arm's ARP entry when the link idles,
    # and its ARP re-queries go unanswered — the PC then gets ETIMEDOUT on :80
    # and :81 until the Pico transmits something.  A tiny UDP packet to the
    # gateway every 15 s keeps the ARP entry and the WiFi association fresh.
    # (Port 9 = discard; nothing needs to listen — the packet itself is the fix.)
    ka_sock        = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ka_gateway     = net.ifconfig()[2]
    last_keepalive = 0
    ...
    while True:
                        ...
                        # visual servo).  Applied at the NEXT START_/move.
                            ...
                            if len(parts) == 3:
                                try:
                                    ...
                                    pass
                        # ── Real-time target: SET_<axis>_<angle> (non-blocking) ──
                        # Point one joint at an absolute angle and return at once,
                        # so a camera loop can refresh targets every frame.
                            ...
                            if len(parts) == 3:
                                try:
                                    ...
                                    if 1 <= ax <= 3:
                                        limit_triggered = False
                                        set_joint_target(all_joints[ax - 1], ax, ang)
                                except ValueError:
                                    pass
                        # ── Real-time full pose: POSE_<a1>_<a2>_<a3> (non-blocking) ──
                        elif cmd.startswith("POSE_"):
                            parts = cmd.split("_")
                            if len(parts) == 4:
                                try:
                                    a1 = float(parts[1])
                                    ...
                                    a3 = float(parts[3])
                                    limit_triggered = False
                                    set_joint_target(joint1, 1, a1)
                                    set_joint_target(joint2, 2, a2)
                                    set_joint_target(joint3, 3, a3)
                                except ValueError:
                                    pass
        ...
        if tcp_client and time.ticks_diff(now, last_tcp_send) > 100:
            ...
            try:
                ...
                # Transient buffer-full under heavy real-time command load: skip
                # this telemetry frame, keep the link.  A genuine disconnect is
                # still caught on the recv() path (empty recv -> close), so we do
                # NOT drop the connection here.
                pass

        # ── Gateway keepalive (see init above) ────────────────────────────────
        if netboot.mode == 'sta' and time.ticks_diff(now, last_keepalive) > 15000:
            last_keepalive = now
            try:
                ka_sock.sendto(b'ka', (ka_gateway, 9))
            except OSError:
                pass
        ...
        # ── Real-time limit guard ─────────────────────────────────────────────
        # A tripped limit switch halts any in-flight non-blocking target move
        # (the blocking jog/move handlers stop themselves; this covers SET_/POSE_).
        if limit_triggered or limit_pressed():
            stop_all_motors()
